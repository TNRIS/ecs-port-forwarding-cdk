...
export class ecsStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const fargateSG = new ec2.SecurityGroup(this, 'FargateServiceSecurityGroup', {
      vpc,
    ...
    // Private Subnet
    const privateSubnet = vpc.selectSubnets({ subnetType: ec2.SubnetType.PRIVATE_WITH_EGRESS }).subnetIds[0];
    ...
    redshiftSG.addIngressRule(fargateSG, ec2.Port.tcp(PORT), 'Allow access from fargate SG');

    // Add outbound rule to allow traffic from fargate SG to Redshift SG
    fargateSG.addEgressRule(redshiftSG, ec2.Port.tcp(PORT), 'Allow access to Redshift SG');
